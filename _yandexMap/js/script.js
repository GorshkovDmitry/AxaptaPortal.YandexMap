//Service constants
//var SERVICE_URL = "http://mascon2009:8080/AxaptaProxyService/"; //Перенес в html - чтобы запускать с разных баз 25.04.2016, GorDS
//const SERVICE_URL = "http://localhost:8733/AxaptaProxyService/";
...
var POLIGON_OPTIONS_SET_ID = "#poligon-options-set";
var POLIGON_DELETE_ID = "#poligon-del";
var FACTORY_NAME_ID = "#factory-name";
var POLIGON_ADD_ID = "#poligon-add"; //02.03.2016, GorDS

//20.05.2016, GorDS -->
var POLIGON_COORDS_OUT = "#poligon-coords-out" 	//Внешний контур
var POLIGON_COORDS_IN = "#poligon-coords-in" 	//Внутренний контура
//20.05.2016, GorDS <--
...
var poligonId = 0;
var poligonNextId = 0;
var colorCurrent = '#ff001a';
var reviewMode = false;

//02.03.2016, GorDS -->
var lastFactory = null; //Последний выбранный завод в списке
var zoneCurrent = null; //Текущая зона
var zonesFactory = null; //Все доступные зоны завода
//02.03.2016, GorDS  <--
...
$(document).ready(function() {
	$(POLIGON_DELETE_ID).prop("disabled", "disabled");
	$(POLIGON_OPTIONS_ID).css( "display", "none");
	$(POLIGON_NAME_ID).prop("disabled", "disabled");
	$(POLIGON_NAME_ID).val("");
	$(POLIGON_OPTIONS_SET_ID).prop("disabled", "disabled");
	$(POLIGON_COLOR_ID).colpick({
		flat:true,
		layout:'hex',
		submit:0,
		color:colorCurrent,
...
function addPoligon()
{	
	alert("Выберите цвет зоны и выделите ее на карте"); //Подсказка 02.03.2016, GorDS
	
	poligonNextId++;
	
	//Из массива зон завода 02.03.2016, GorDS -->
	var zoneId = parseInt($(POLIGON_SELECT_ID).val());
	var poligonName = zonesFactory[zoneId];	
	//Из массива зон завода 02.03.2016, GorDS <--
	
	/*Закомментировал 02.03.2016, GorDS 
	var poligonName = "#" + poligonNextId;

	//Add new option to select
	$(POLIGON_SELECT_ID)
         .append($("<option></option>")
         .attr("value", poligonNextId)
         .text(poligonName));
	$(POLIGON_SELECT_ID + " option[value=" + poligonNextId + "]").prop("selected", "selected");
	Закомментировал 02.03.2016, GorDS */
	
	poligonId = poligonNextId;
	
	removePoligon();
	
	//Create new polygon
	poligon = new ymaps.GeoObject({
        geometry: {
            type: "Polygon",
            coordinates: []
        },
		properties: {
			balloonContent: poligonName
		}
    }, {
...
function clearMap() //Очистка карты 02.03.2016, GorDS 
{
	...
	$(POLIGON_SELECT_ID + " option").remove();
	$(POLIGON_SELECT_ID)
         .append($("<option></option>")
         .attr("value", "0")
         .text("Не выбрано"));
...
{
	if (poligon == null || poligonId == 0) return;
	
	//$(POLIGON_SELECT_ID + " option[value='" + poligonId + "']").remove(); //Закомментировал 02.03.2016, GorDS 
	
	map.geoObjects.remove(poligon);
	poligon = null;
	poligons[poligonId] = null;
	poligons.splice(poligonId, 1); //Удаление элемента 02.03.2016, GorDS 
	
	$(POLIGON_SELECT_ID).val("0"); 
	choosePoligon();
}
...
//Select polygon
function choosePoligon()
{
	// 02.03.2016, GorDS -->
	//poligonId = parseInt($(POLIGON_SELECT_ID).val()); //закомментировал
	var zoneId = parseInt($(POLIGON_SELECT_ID).val());
	var poligonName = zonesFactory[zoneId];	
	zoneCurrent = poligonName;
	
	poligonId = 0;
	
	if (poligonName != '' && poligonName != null)
	{
		for (var i = 1; i < poligons.length; ++i)
		{
			if (poligons[i].properties.get('balloonContent') == poligonName)
			{
				poligonId = i;
				break;
			}
...
function setPoligonOptions()
{
	if (poligon == null) return;
	
	var poligonName = $.trim($(POLIGON_NAME_ID).val());
	if (!poligonName)
	{
		poligonName = "#" + poligonId;
	}
	
	poligon.options.set('fillColor', colorCurrent);
	poligon.properties.set('balloonContent', poligonName);
	
	$(POLIGON_SELECT_ID + " option:selected").text(poligonName);
}

//Polygon parameters controls state
function controlsPoligon()
{
	if (poligon == null)
	{
		$(POLIGON_DELETE_ID).prop("disabled", "disabled");
		//$(POLIGON_ADD_ID).prop("disabled", ""); //Работа кнопки "Добавить на карте" 02.03.2016, GorDS //task21182 Закомментировал 17.08.2016, GorDS
		$(POLIGON_OPTIONS_ID).css( "display", "none");
		$(POLIGON_NAME_ID).prop("disabled", "disabled");
		$(POLIGON_OPTIONS_SET_ID).prop("disabled", "disabled");
	...
	{
		$(POLIGON_DELETE_ID).prop("disabled", "");
		//$(POLIGON_ADD_ID).prop("disabled", "disabled"); //Работа кнопки "Добавить на карте" 02.03.2016, GorDS //task21182 Закомментировал 17.08.2016, GorDS
		$(POLIGON_OPTIONS_ID).css( "display", "block");
		$(POLIGON_NAME_ID).prop("disabled", "");
		$(POLIGON_OPTIONS_SET_ID).prop("disabled", "");
		
		$(POLIGON_NAME_ID).val(poligon.properties.get('balloonContent'));
		$(POLIGON_COLOR_ID).colpickSetColor(poligon.options.get('fillColor'), poligon.options.get('fillColor'));
	}
	
	// 02.03.2016, GorDS -->
	if (zoneCurrent == null)
	{
		$(POLIGON_ADD_ID).prop("disabled", "disabled"); //Работа кнопки "Добавить на карте" 
	}
	//task21182 17.08.2016, GorDS -->
	else
	{
		$(POLIGON_ADD_ID).prop("disabled", "");
	}
	//task21182 17.08.2016, GorDS <--
	// 02.03.2016, GorDS <--
}

function confirmEditedPoligons() //Перед изменением завода спрашиваем о переходе 02.03.2016, GorDS 
{
	if (lastFactory != "Не выбрано" && lastFactory != null)
		return confirm("Все несохраненные изменения будут потеряны. Изменить завод?");
	else
		return true;
}

//Save polygons to factory
function saveFactory()
{
	var factoryName = $.trim($(FACTORY_NAME_ID).val());
	if (!factoryName)
	{
		alert("Не указано имя завода!");
		return;
	}
	
	var savePoligons = new Array();
	for (var i = 1; i < poligons.length; ++i)
	{
		if (poligons[i] != null)
		{
			savePoligons.push({
				Name: poligons[i].properties.get('balloonContent'),
				Color: poligons[i].options.get('fillColor'),
				Coords: ymaps.geometry.Polygon.toEncodedCoordinates(poligons[i].geometry)
			});
		}
	}
	
	if (savePoligons.length == 0)
	{
		alert("Не создано ни одной зоны!");
		return;
	}
	...
	var json = {
		...
		Zoom: map.getZoom(),
		Poligons: savePoligons
	};
	var jsonString = JSON.stringify(json);
	var jsonBase64String = Base64.encode(JSON.stringify(json));
	
	//Ajax request. Send json of factory zones.
	var xmlHttp = new ActiveXObject("Microsoft.XmlHttp");
	var url = SERVICE_URL + "AJAX-Endpoint/SetYandexZone";
	var body = '{"jsonBase64String":"' + jsonBase64String + '"}';
...
function loadFactory()
{
	...
	//02.03.2016, GorDS -->	
	lastFactory = factoryName; //Сохранение последнего введенного завода
	
	if (factoryName == "Не выбрано")
	{
		clearMap();
		return;
	}
	...
	//Ajax request. Get json of factory zones.
	var xmlHttp = new ActiveXObject("Microsoft.XmlHttp");
	var url = SERVICE_URL + "AJAX-Endpoint/GetYandexZone";
	var body = '{"factory":"' + factoryName + '"}';
	...
	xmlHttp.onreadystatechange= function X()
	{
		 if(xmlHttp.readyState == 4)
		 {
			try
			{
				...
				var jsonBase64String = jsonAnswer.d;
				
				//02.03.2016, GorDS -->
				if (!jsonBase64String)
				{
					clearMap();
					alert("Для данного завода нет доступных зон.");
					return;
				}
				//02.03.2016, GorDS <--
				
				var jsonString = Base64.decode(jsonBase64String);
				
				loadFactoryFromJSONString(jsonString);
			}
			catch (ex)
			{
				alert("Ошибка при чтении ответа от сервера. Обратитесь к администратору.");
			}
		 }
	}
	...
	$.ajax({
		...
		url: SERVICE_URL + "AJAX-Endpoint/GetYandexZone",
...
function loadFactoryFromJSONString(jsonString)
{
	...
	/* Перенес в метод 02.03.2016, GorDS -->
	//Reseting workplace
	for (var i = 1; i < poligons.length; ++i)
	{
		if (poligons[i] != null)
	...
	//Loading factory zones
	map.setCenter(json.Center, json.Zoom);
	var loadPoligons = json.Poligons;
	for (var i = 0; i < loadPoligons.length; ++i)
	{
		var loadPoligon = new ymaps.Polygon(ymaps.geometry.Polygon.fromEncodedCoordinates(loadPoligons[i].Coords));
		...
		$(POLIGON_SELECT_ID)
         .append($("<option></option>")
         .attr("value", poligonNextId)
         .text(loadPoligons[i].Name));
		Закомментировал 02.03.2016, GorDS */
	}
}

function addFactoriesToControl(factories) //Добавление заводов (Подразделений) к списку 02.03.2016, GorDS 
{
	for (var i = 0; i < factories.length; ++i)
	{		
		$(FACTORY_NAME_ID)
         .append($("<option></option>")
         .attr("value", factories[i].Factory)
         .text(factories[i].Factory));
	}
}

function initFactories() //Вывод всех заводов (Подразделений) 02.03.2016, GorDS 
{
	//Ajax request
	var xmlHttp = new ActiveXObject("Microsoft.XmlHttp");
	var url = SERVICE_URL + "AJAX-Endpoint/GetWebFactories";
	...
	xmlHttp.onreadystatechange= function X()
	{
		 if(xmlHttp.readyState == 4)
		 {
			...
			{
				var jsonAnswer = JSON.parse(xmlHttp.responseText);
				var factories = jsonAnswer.d;
				addFactoriesToControl(factories);
			}
			catch (ex)
			{
				alert("Ошибка при чтении ответа от сервера. Обратитесь к администратору.");
...
function addZonesToControl(zones) //Вывод всех заводов (Подразделений) 02.03.2016, GorDS 
{
	var j = 1;
	for (var i = 0; i < zones.length; ++i)
	{		
		//Выбираем зоны текущего завода
		if (zones[i].Department == lastFactory && zones[i].Name != '')
		{
			zonesFactory[j] = zones[i].Name;  
...
function loadZonesFactory() //Загрузка доступных зон завода 02.03.2016, GorDS 
{
	zonesFactory = new Array();
	zoneCurrent  = null;
	
	//Ajax request
	var xmlHttp = new ActiveXObject("Microsoft.XmlHttp");
	var url = SERVICE_URL + "AJAX-Endpoint/GetZones";
	...
	xmlHttp.onreadystatechange= function X()
	{
		 if(xmlHttp.readyState == 4)
		 {
			...
			{
				var jsonAnswer = JSON.parse(xmlHttp.responseText);
				var zones = jsonAnswer.d;
				alert("Выбран завод " + lastFactory); //TODO Без него не работает
				addZonesToControl(zones);
			}
			catch (ex)
			{
				alert("Ошибка при чтении ответа от сервера. Обратитесь к администратору.");
...
function changeFactory() //Изменение завода в списке 02.03.2016, GorDS 
{
	//Перед изменением завода спрашиваем о переходе
	if (!confirmEditedPoligons()) 
	{
		$.trim($(FACTORY_NAME_ID).val(lastFactory)); 
		return;
	}
...
function getCoords() //Показать координаты зоны 19.05.2016, GorDS 
{
	var outCoords = ArrayParser(poligon.geometry.get(0));
	var inCoords;
	
	if (poligon.geometry.get(1))
	{
		inCoords = ArrayParser(poligon.geometry.get(1));
	}

	$(POLIGON_COORDS_OUT).val(outCoords); 
	$(POLIGON_COORDS_IN).val(inCoords);
}

function updateCoords() //Обновить координаты зоны 19.05.2016, GorDS 
{
	//Преобразуем строку в массив по элементно 
	var outCoords = ArrayParser($(POLIGON_COORDS_OUT).val());
	var inCoords = ArrayParser($(POLIGON_COORDS_IN).val());
	
	poligon.geometry.set(0, outCoords);
	poligon.geometry.set(1, inCoords);
}


function ArrayParser(data){ //Преобразование строки в многомарный массив и обратно 19.05.2016, GorDS 
	var x =",";
	var y =";";
...
//Onload map
ymaps.ready(init);
initFactories(); //Вывод всех заводов (Подразделений) 02.03.2016, GorDS 